...
$search_defdb = GETPOST('search_defdb', 'alpha');
...
$sql = "SELECT mu.rowid as uid, u.lastname, u.firstname, u.login, u.email, u.statut as status, mu.default_db, mu.date_creation, u.datec ";
$sql .= " FROM ".MAIN_DB_PREFIX."multidb_mudb_users as mu";
$sql .= " JOIN ".$object->ref.".".MAIN_DB_PREFIX."user as u on mu.fk_remote_user = u.rowid";
$sql .= " WHERE mu.ref_db = '".$object->id."'";


// Search criteria
if ($search_ref) {
	$sql .= " AND mu.rowid=".((int) $search_ref);
}
/*if ($search_user) {
	$sql .= natural_search(array('u.login', 'u.lastname', 'u.firstname', 'u.email'), $search_user);
}*/
if ($search_label) {
	$sql .= natural_search(array('u.login'), $search_label);
}
if (!empty($search_date_start_from) /*&& !empty($search_date_start_to)*/) {
	$sql .= " AND DATE(mu.date_creation) = DATE('".$db->idate($search_date_start_from)."')";
}
...
if ($search_status != '' && $search_status >= 0) {
	$sql .= " AND u.statut = ".((int) $search_status);
}
if ($search_defdb  != '' ) {
	$sql .= " AND mu.default_db = '". $search_defdb."'";
}
...
print dol_get_fiche_head($head, 'users', $langs->trans("Workstation"), -1, $object->picto);
...
print '<input class="flat" type="text" size="6" name="search_defdb" value="'.$db->escape($search_defdb).'">';
print '</td>';

// Status
print '<td class="liste_titre left">';
$object->select_statuses_user($search_status, 'search_status', '', 0, 1, 1, 16);
print '</td>';
...
print_liste_field_titre("Ref", $_SERVER["PHP_SELF"], "mu.rowid", "", $param, "", $sortfield, $sortorder);
print_liste_field_titre("Login", $_SERVER["PHP_SELF"], "u.login", "", $param, 'class="left"', $sortfield, $sortorder);
print_liste_field_titre("DateCreation", $_SERVER["PHP_SELF"], "u.datec", "", $param, 'align="center"', $sortfield, $sortorder);
//print_liste_field_titre("DateEnd", $_SERVER["PHP_SELF"], "s.dateep,s.rowid", "", $param, 'align="center"', $sortfield, $sortorder);
print_liste_field_titre("BDPrincipal", $_SERVER["PHP_SELF"], "mu.default_db", "", $param, "", $sortfield, $sortorder);
print_liste_field_titre("Status", $_SERVER["PHP_SELF"], "u.statut", "", $param, 'class="left"', $sortfield, $sortorder);
/*if (!empty($conf->banque->enabled)) {
...
//SELECT mu.rowid as uid, u.lastname, u.firstname, u.login, u.email, u.statut as status, mu.default_db, mu.date_creation, u.datec
...
while ($i < ($limit ? min($num, $limit) : $num)) {
	...
	// Status
	print '<td>';
	$Statu_ = $obj->status ? "ACTIVO" : "INACTIVO";
	print $Statu_;
